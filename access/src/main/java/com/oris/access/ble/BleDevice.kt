...
abstract class BleDevice<T>(val device: BluetoothDevice) : BluetoothGattCallback(){
    ...
    fun destroy() {
        OLog.d(TAG, "destroy the connected $mDeviceStatus")
        if (mDeviceStatus and MASK_VALID_STATUS == 0) {
            OLog.d(TAG, "destroy the connected")
            mGatt?.close()
            mGatt = null
            mDeviceStatus = STATUS_DESTORYED
        } else {
            OLog.d(TAG, "destroy the connected status incorrect")
        }
    }
    ...
    fun write(characteristic: BluetoothGattCharacteristic):Boolean {
        OLog.d(TAG, "write-chara ${StringUtils.toHexString(characteristic.value)}")
    ...
    override fun onConnectionStateChange(gatt: BluetoothGatt?, status: Int, newState: Int) {
        ...
        OLog.d(TAG, "onConnectionStateChange status:$status newState:$newState")
        if (BluetoothGatt.GATT_SUCCESS == status) {
            // CQ :status 表示相应的连接或断开操作是否完成，而不是指连接状态
            if (newState == BluetoothGatt.STATE_DISCONNECTED) {
                mDeviceStatus = STATUS_DISCONNECTED
                OLog.d(TAG, "onConnectionStateChange disconnected")
                onDevicesDisconnected()
            } else if (newState == BluetoothGatt.STATE_CONNECTED) {
                OLog.d(TAG, "onConnectionStateChange connected")
            ...
            OLog.d(TAG, "onConnectionStateChange status failed")
            release()
            onDevicesDisconnected()
        }
    }
    ...
    override fun onServicesDiscovered(gatt: BluetoothGatt?, status: Int) {
        OLog.d(TAG, "onServicesDiscovered status $status discovered services")
        if (status == BluetoothGatt.GATT_SUCCESS) {
            gatt?.services?.let { its ->
                for (item in its.iterator()){
                    OLog.d(TAG, "services status ${item.uuid}")
    ...
    override fun onCharacteristicRead(gatt: BluetoothGatt?, characteristic: BluetoothGattCharacteristic?, status: Int) {
        if(BluetoothGatt.GATT_SUCCESS == status){
            OLog.d(TAG,  "on-chara-read value=${StringUtils.toHexString(characteristic?.value)} success")
        }else{
            OLog.d(TAG,  "on-chara-read value=${StringUtils.toHexString(characteristic?.value)} failed")
        }
    }

    override fun onCharacteristicWrite(gatt: BluetoothGatt?, characteristic: BluetoothGattCharacteristic?, status: Int) {
        if(BluetoothGatt.GATT_SUCCESS == status){
            OLog.d(TAG,  "on-chara-write value=${StringUtils.toHexString(characteristic?.value)} success")
        }else{
            OLog.d(TAG,  "on-chara-write value=${StringUtils.toHexString(characteristic?.value)} failed")
        }
    }

    @Synchronized
    override fun onCharacteristicChanged(gatt: BluetoothGatt?, characteristic: BluetoothGattCharacteristic?) {
        OLog.d(TAG, "onCharacteristicChanged value=${StringUtils.toHexString(characteristic?.value)}")
    ...
    fun close() {
        OLog.d(TAG, "device close connection")
        mGatt?.close()
        mDeviceStatus = STATUS_DISCONNECTED
    }
}